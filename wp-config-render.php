 ...
 * Uses SQLite via sqlite-database-integration plugin
 */

// ---- ABSPATH must be defined first ----
if ( ! defined( 'ABSPATH' ) ) {
    define( 'ABSPATH', __DIR__ . '/' );
}

// ---- SQLite settings (hardcoded path — avoids WP_CONTENT_DIR undefined error) ----
define( 'DB_DIR',  ABSPATH . 'wp-content/database/' );
define( 'DB_FILE', 'wordpress.db' );

// ---- Database placeholders (SQLite plugin overrides these) ----
...
// ---- Site URL (auto-detect HTTPS on Render) ----
if ( isset( $_SERVER['HTTP_HOST'] ) ) {
    $protocol = ( ! empty( $_SERVER['HTTPS'] ) && $_SERVER['HTTPS'] !== 'off' ) ? 'https' : 'http';
    // Render uses X-Forwarded-Proto
