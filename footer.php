<?php
/**
 * Footer template — Smart Sports Theme
 * Contact info editable via Appearance → Customize → ⑥ Footer
 */
?>
<footer class="site-footer" role="contentinfo" id="site-footer">
    <div class="container">
        <div class="footer-grid">

            <!-- Col 1: Brand -->
            <div class="footer-col">
                <div class="footer-logo">
                    <img
                        src="<?php echo esc_url( get_template_directory_uri() . '/image/logo.png' ); ?>"
                        alt="Smart Sports"
                        style="height:80px;width:auto;display:block;"
                    />
                </div>
                <p>Smart Sports is a registered 501 (c)3 nonprofit</p>
                <a href="#contact" class="btn btn-green" style="font-size:14px;padding:10px 22px;text-transform:none;font-weight:600;letter-spacing:0;">Support the work</a>
            </div>

            <!-- Col 2: Quick Links -->
            <div class="footer-col">
                <h4 style="text-transform:none; font-family:var(--font-main); color:var(--color-blue-mid); font-size:15px; font-weight:700; letter-spacing:0;">Quick links</h4>
                <ul>
                    <li><a href="#about">About Us</a></li>
                    <li><a href="#how-it-works">How It Works</a></li>
                    <li><a href="#">Where We Work</a></li>
                    <li><a href="#impact">Impact</a></li>
                    <li><a href="#contact">Get Involved</a></li>
                </ul>
            </div>

            <!-- Col 3: Resources + Social -->
            <div class="footer-col">
                <h4 style="text-transform:none; font-family:var(--font-main); color:var(--color-blue-mid); font-size:15px; font-weight:700; letter-spacing:0;">Resources</h4>
                <ul>
                    <li><a href="#">News</a></li>
                    <li><a href="#">Events</a></li>
                    <li><a href="#">Blogs</a></li>
                </ul>

                <h4 style="margin-top:24px; text-transform:none; font-family:var(--font-main); color:var(--color-blue-mid); font-size:15px; font-weight:700; letter-spacing:0;">Follow Us</h4>
                <div class="social-icons">
                    <?php
                    $socials = array(
                        array( 'key' => 'ss_ig', 'label' => 'Instagram',  'default' => '#',
                               'svg' => '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="18" height="18"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg>' ),
                        array( 'key' => 'ss_li', 'label' => 'LinkedIn',   'default' => '#',
                               'svg' => '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="18" height="18"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></svg>' ),
                    );
                    foreach ( $socials as $s ) :
                        $href = get_theme_mod( $s['key'], $s['default'] );
                    ?>
                    <a href="<?php echo esc_url( $href ); ?>" class="social-icon-wrap" aria-label="<?php echo esc_attr( $s['label'] ); ?>" target="_blank" rel="noopener">
                        <?php echo $s['svg']; ?>
                    </a>
                    <?php endforeach; ?>
                </div>
            </div>

            <!-- Col 4: Contact -->
            <div class="footer-col">
                <h4 style="text-transform:none; font-family:var(--font-main); color:var(--color-blue-mid); font-size:15px; font-weight:700; letter-spacing:0;">Contact</h4>

                <div class="contact-item">
                    <span class="contact-icon">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/>
                        </svg>
                    </span>
                    <span style="color:var(--color-text);"><?php echo nl2br( esc_html( get_theme_mod('ss_address','3340 N Hamilton Ave,
Unit 1, Chicago, IL 60618') ) ); ?></span>
                </div>

                <div class="contact-item">
                    <span class="contact-icon">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.56 3.44 2 2 0 0 1 3.54 1.27h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L7.91 8.91a16 16 0 0 0 6 6l.91-.91a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 21.73 16.92z"/>
                        </svg>
                    </span>
                    <span style="color:var(--color-text);"><?php echo esc_html( get_theme_mod('ss_phone','773-705-3102') ); ?></span>
                </div>

                <div class="contact-item">
                    <span class="contact-icon">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/>
                        </svg>
                    </span>
                    <a href="mailto:<?php echo esc_attr( get_theme_mod('ss_email','info@mysmartsports.org') ); ?>" style="color:var(--color-text);">
                        <?php echo esc_html( get_theme_mod('ss_email','info@mysmartsports.org') ); ?>
                    </a>
                </div>
            </div>

        </div><!-- /.footer-grid -->
    </div><!-- /.container -->

    <!-- Bottom bar -->
    <div class="footer-bottom">
        <div class="container">
            <p style="color:var(--color-blue-dark); font-weight:500; font-size:15px;">&copy; <?php echo date('Y'); ?> Smart Sports. All rights reserved.</p>
        </div>
    </div>

</footer>

<?php wp_footer(); ?>
</body>
</html>
